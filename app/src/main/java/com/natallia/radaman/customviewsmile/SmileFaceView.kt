...
/**
 * Created by Natallia Radaman on 12.04.2018.
 */

class SmileFaceView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    companion object {
    ...
    private var size = 0
    val INSTANCE_STATE = "instanceState"
    val INSTANCE_SMILE = "instanceSmile"
    ...
    override fun onSaveInstanceState(): Parcelable {
        ...
        bundle.putLong(INSTANCE_SMILE, happinessState)
        return bundle
    }
    ...
    override fun onRestoreInstanceState(state: Parcelable?) {
        if (state is Bundle) {
            with(state) {
                happinessState = getLong(INSTANCE_SMILE)
            }
